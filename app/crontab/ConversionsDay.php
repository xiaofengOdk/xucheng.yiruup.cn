...
Db::connection('mysql2')->table('baidu_xinxiliu_project_list')->select('id', 'projectName','hid', 'loginurl','login_name','login_pwd','project_type', 'conversions_day')
                ...
                try {
                    ...
                    if (strpos($login_success, 'main') !== false) {
                        ...
                        */
                        ...
                        var_dump($project_list_item->projectName."已加粉".$totalSum."总加粉".$project_list_item->conversions_day);

                        if ($totalSum   >= $project_list_item->conversions_day) {
                            // 优化：先过滤出有效的项目
                            $validProjects = array_filter($project, function($item) {
                                return $item['types'] == 2 && !($item['budget'] > 0 && $item['userStat'] == 11);
                            });
                            if (!empty($validProjects)) {
                                foreach($validProjects as $project_item){
                                    $updatetime = Redis::get('project_converday_' . $date . $project_item['id']);
                                    if(!$updatetime){
                                        // 将对象转换为数组，给CampaginBaidu模型使用

                                        $campaignFeed = $responseCampaginBaidu->getCampaignFeedCrontab([], $project_item, $project_item['accessToken'], null, $startsTime, $endsTime);
                                        $message_error_success .= $project_item['userName'].$campaignFeed['message']."\n";
                                        if($campaignFeed['code']==0){
                                            Redis::set('project_converday_' . $date . $project_item['id'], 1);
                                            Log::channel('datareportjiafen')->info('百度账号[' . $project_item['userName'] . '] -成功 项目[' . $project_item['clientName'] . ']'.$campaignFeed['message']).'时段'.$startsTime.'-'.$endsTime;


                                        } else {
                                             Redis::set('project_converday_' . $date . $project_item['id'], 1);
                                            Log::channel('datareportjiafen')->info('百度账号[' . $project_item['userName'] . '] - 失败项目[' . $project_item['clientName'] . ']'.$campaignFeed['message']);
                                        }
                                    }
                                }
                            }
                                    ...
                                    foreach($validProjects as $project_item){
                                        ...
                                        if(!$updatetime){
                                            ...
                                            if($campaignFeed['code']==0){
                                                ...
                                                Log::channel('datareportjiafen')->info('百度账号[' . $project_item['userName'] . '] - 成功项目[' . $project_item['clientName'] . ']'.$campaignFeed['message']).'时段'.$startsTime.'-'.$endsTime;
                            ...
                            if ($totalSum  >= $project_list_item->conversions_day) {
                                ...
                                if (!$is_push) {
                                    ...
                                    $weixin_message = "项目：<font color=\"info\">" . $project_list_item->projectName . "</font> 现加粉总数为<font color=\"info\">" . $totalSum . "</font>。
                                             >快接近于日加粉量<font color=\"comment\">" . $project_list_item->conversions_day . "</font>。
                                             >请<font color=\"info\">相关同事注意</font>。
                                             >内容<font color=\"comment\">" . $message_error_success . "</font>。
                                             >时段<font color=\"comment\">" . $startsTime.'-'.$endsTime . "</font>。
                                             >时间: " . date("Y-m-d H:i:s") . "。\n";
                                    if (isset($admins[$project[0]['sellId']]))
                                        $weixin_message .= ">销售: <font color=\"info\">" . $admins[$project[0]['sellId']]->userName . " </font>。\n";
