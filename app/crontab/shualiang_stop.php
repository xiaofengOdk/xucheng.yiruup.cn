<?php
//有刷量 假量的 微信通知 2分钟运行一次计划任务 直接停信息流账户计划
...
$time_before_30_min = date('Y-m-d H:i:s', strtotime('-1 minutes'));
        ...
        foreach ($subuser_map as $subuser => $count) {
            ...
            foreach ($logidUrl_map[$subuser] as $k => $_l) {
                if ($dataById_map[$k]->ispush == 0) {
                    ...
                    curl_setopt($ch, CURLOPT_TIMEOUT, 3); // 请求超时时间（秒）
