...
if (!function_exists('getAdgroupFeed2')) {
    function getAdgroupFeed2($user_payload, $ids)
    {
...
//查询单元
$adgroupFeedData = getAdgroupFeed2($user_payload, $campaignFeedIds);
//如果单元数据查询成功
if ($adgroupFeedData['header']['desc'] == 'success') {
    ...
    if (is_array($adgroupFeedData['body']['data']) && count($adgroupFeedData['body']['data']) > 0) {
        ...
        foreach ($adgroupFeedData['body']['data'] as $k => $adgroupFeed) {
            ...
            //推广单元名称
            $data2['adgroupFeedName'] = explode('-', $adgroupFeed['adgroupFeedName'])[0] . mt_rand(100, 10000) . date("_md_H:i:s");
        ...
        if (is_array($adgroupFeedResponseData['body']['data']) && count($adgroupFeedResponseData['body']['data']) > 0) {
            ...
            //新建单元
            require app_path() . '/crontab/creativeFeedType2.php';
        } else {
