...
$userName = 'QD-阑风伏雨';
$toUserName = 'baidu-QDxc九霄';

$xinxiliu_subuser = Db::connection('mysql2')->table('baidu_xinxiliu_subuser')->where('userName', $userName)->first();
$xinxiliu_refreshToken = Db::connection('mysql2')->table('baidu_xinxiliu_refreshToken')->where('userId', $xinxiliu_subuser->masterUid)->first();
...
if ($feedData['header']['desc'] == 'success') {
    //查询要被复制过去的账户的 ocpc
    $toxinxiliu_subuser = Db::connection('mysql2')->table('baidu_xinxiliu_subuser')->where('userName', $toUserName)->first();
    $toxinxiliu_refreshToken = Db::connection('mysql2')->table('baidu_xinxiliu_refreshToken')->where('userId', $toxinxiliu_subuser->masterUid)->first();

    $user_payload = array(
        "header" => array(
            "userName" => $toUserName,
            "accessToken" => $toxinxiliu_refreshToken->accessToken,
            "action" => "API-PYTHON"
        ),
    );
    $tourl = 'https://api.baidu.com/json/sms/service/AssetEventApiService/getAssetDetailList';
    $user_payload['body'] = [
        'pageNo' => 1,
        'pageSize' => 20,
    ];
    $jsonData = json_encode($user_payload);
    $ocpcresponse = getData($tourl, $jsonData);
    $ocpcresponse = json_decode($ocpcresponse, true);
    ...
    if (is_array($feedData['body']['data']) && count($feedData['body']['data']) > 0) {
        ...
        foreach ($feedData['body']['data'] as $k => $CampaignFeedType) {
            $data = [];
            $data['campaignFeedName'] = explode('_', $CampaignFeedType['campaignFeedName'])[0] . date("_md_H:i:s") . mt_rand(100, 1000);
            ...
            $ocpc = $ocpcresponse['body']['data'][0];
            $data["ocpc"]['appTransId'] = $ocpc['transId'];
            $data["ocpc"]['traceAppName'] = $ocpc['transName'];
        ...
        if ($addresponse['header']['desc'] != 'success') {
            Log::channel('feed')->info('账户：' . $userName . ' 计划复制失败,crontab/feed2.php,失败原因: ' . json_encode($addresponse['body']['header']));
        }
        //批量新建计划成功之后，批量新建单元
        if (is_array($addresponse['body']['data']) && count($addresponse['body']['data']) > 0) {
            ...
            //新建单元
             require app_path() . '/crontab/adgroupFeedType2.php';
        } else {
